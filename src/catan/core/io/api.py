    ...
    *,
    ...
    with backend.open_read(path) as ref:
        if fields_to_load is not None:
            ...
            raise ValueError(
                "Either fields_to_load or config_name must be provided."
            )
    ...
    for name, subconfig_spec in config.subconfigs.items():
        ...
        if not backend.exists(
            ...
            if subconfig_spec.required:
                raise KeyError(
                    f"Required subobject {name!r} "
                    f"not found at {subroot!r}"
                )

            data[name] = None
            continue

        subconfig = LoadConfig.load_resource(
            subconfig_spec.config
        )
    ...
    *,
    ...
    for name, subconfig_spec in config.subconfigs.items():
        ...
        if subdata is None:
            if subconfig_spec.required:
                raise ValueError(
                    f"Required subobject {name!r} is missing."
                )
